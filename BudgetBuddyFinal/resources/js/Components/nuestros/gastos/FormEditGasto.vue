...
export default {
    ...
    setup() {
        ...
        const submit = ($id) => {
            form.put(route('gastos.update', $id));
        };
...
<template>
    <div>
        ...
        <div v-if="confirmingCardAddition">
            <form @submit.prevent="submit(gasto.id)" method="put" style="border: 0;">
                <div class="mt-0 py-2 row-auto inline-grid">
                    ...
                    <div class="col-auto me-5">
                        ...
                        <TextInput id="id" v-model="form.id" type="text" class="mt-1 w-full" :value="gasto.id" disabled />
                    </div>
                    <div class="col-auto me-5">
                        <InputLabel for="descripcion" value="Descripcion" />
                        <TextInput id="descripcion"  type="text" class="mt-1 w-full" :value="gasto.descripcion" @input="form.descripcion = $event.target.value" />
                    </div>
                    <div class="col-auto me-5">
                        <InputLabel for="monto" value="Monto" />
                        <TextInput id="monto"  type="text" class="mt-1 w-full" :value="gasto.monto" @input="form.monto = $event.target.value"
                            maxlength="16" />
                    </div>
                    <div class="col-auto me-5">
                        <InputLabel for="fecha" value="Fecha" />
                        <TextInput id="fecha" type="date" class="mt-1 w-full" :value="gasto.fecha" @input="form.fecha = $event.target.value"/>
                    </div>
